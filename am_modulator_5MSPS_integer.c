...
int main(int argc, char *argv[]) {
    ...
    if (optind >= argc) {
        ...
        fprintf(stderr, "  Erweitert: %s -b 16 -s 12.5 1234:603000 1235:828000\n", argv[0]);
        return 1;
    }
    ...
    if (sample_rate_msps == 10.0f) {
        // Exaktes Original-Verhalten: 5 MSPS NCO Rate, Upsampling 200, Faktor 2 Verdopplung am Ausgang
        ...
        current_upsample = current_inter_rate / 25000; // e.g. 12.5M / 25k = 500
        out_multiplier = 1;
    }
    ...
    for (int i = 0; i < num_transmitters; i++) {
        ...
        // NCO Phase Increment basierend auf dynamischer NCO Clock (current_inter_rate)
    ...
    // Dynamische Puffergröße um auch 25 MSPS (1000er Upsample) bei 16 Bit zu puffern.
    // Maximal benötigte Samples: Audio-Chunk (500) * Upsample (max 1000) * Multiplikator (2)
    // = 1.000.000 Samples. * 2 Byte bei 16-Bit = 2 Megabyte
    size_t dac_max_samples = AUDIO_CHUNK * 1000 * 2;
    ...
    while(1) {
        ...
        // ====================================================================
        unsigned int out_idx = 0; // Index zählt jetzt in Samples (nicht zwingend in Bytes)
        ...
        for (int a = 0; a < AUDIO_CHUNK; a++) {
            ...
            for (int i = 0; i < num_transmitters; i++) {
                // Träger-Basisamplitude + moduliertes Signal
            ...
            for (int u = 0; u < current_upsample; u++) {
                ...
                // Ausgabe (1x für neue Raten, 2x für alte 10 MSPS Rückwärtskompatibilität)
